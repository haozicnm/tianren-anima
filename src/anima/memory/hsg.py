...
from ..core.constants import SECTOR_CONFIGS, NATURE_DECAY_MODIFIERS
...
from ..utils.keyword import compute_keyword_overlap
...
from .decay import inc_q, dec_q, on_query_hit, calc_recency_score as calc_recency_score_decay
...
async def embed_query_for_all_sectors(query: str, sectors: List[str]) -> Optional[Dict[str, List[float]]]:
    """Embed query for all sectors. Returns None if embedding service is unavailable."""
    try:
        res = {}
        for s in sectors:
        ...
        logger.warning("hsg: embedding unavailable (%s), will use BM25 fallback", e)
        return None


def bm25_fallback_search(query: str, sectors: List[str], k: int, user_id: Optional[str] = None) -> Dict[str, List[Dict[str, Any]]]:
    """BM25-style full-text search fallback when embedding service is down.

    Uses SQL LIKE + keyword token overlap for ranking. No vectors needed.
    """
    # Extract search tokens (lowercase, deduplicated)
    tokens = set(re.findall(r'[\w\u4e00-\u9fff]+', query.lower()))
    if not tokens:
        return {s: [] for s in sectors}

    # Build SQL: search memories whose content matches any token
    user_filter = " AND user_id=?" if user_id else ""
    params_base = [user_id] if user_id else []

    # Use multiple LIKE conditions for broader recall
    like_clauses = []
    like_params = []
    for tok in list(tokens)[:10]:  # cap at 10 tokens to avoid huge queries
    ...
    for sector in sectors:
        sql = f"""SELECT id, content, nature, primary_sector, salience, last_seen_at
                  FROM memories
                  WHERE ({where}){user_filter}
                  ORDER BY last_seen_at DESC
                  LIMIT ?"""
        params = [*like_params, *params_base, k * 3]
        try:
        ...
        for r in rows:
            ...
            # BM25-like score: token overlap ratio
            mem_tokens = set(re.findall(r'[\w\u4e00-\u9fff]+', (content or "").lower()))
            if not mem_tokens:
                continue
            overlap = len(tokens & mem_tokens) / len(tokens)
            scored.append({"id": mid, "similarity": overlap})

        scored.sort(key=lambda x: x["similarity"], reverse=True)
        results[sector] = scored[:k * 3]
...
def calc_decay(sec: str, init_sal: float, days_since: float, seg_idx: Optional[int] = None, max_seg: Optional[int] = None, nature: Optional[str] = None) -> float:
    cfg = SECTOR_CONFIGS.get(sec)
    if not cfg: return init_sal
    lam = cfg["decay_lambda"]
    # Apply nature-based decay modifier (L2 Heavenly-Human Harmony)
    if nature and nature in NATURE_DECAY_MODIFIERS:
        lam = lam * NATURE_DECAY_MODIFIERS[nature]
...
async def add_hsg_memory(content: str, tags: Optional[str] = None, metadata: Any = None, user_id: Optional[str] = None, nature: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # P0.2: Extract and link temporal facts from memory content
...
async def hsg_query(qt: str, k: int = 10, f: Dict[str, Any] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        sr = {}
        # L3 群智融合: altruistic queries search across all users
        search_uid = None if (f and f.get("nature") == "altruistic") else f.get("user_id") if f else None

        if qe is not None:
            # Normal path: vector search
            for s in ss:
                qv = qe[s]
                res = await store.search(qv, s, k*3, {"user_id": search_uid})
                sr[s] = res
        else:
            # Degraded path: BM25 fallback (no embedding needed)
            logger.info("hsg: degraded search mode (BM25 + graph)")
            sr = bm25_fallback_search(qt, ss, k, search_uid)
        ...
        for mid in ids:
            ...
            sal = calc_decay(m["primary_sector"], m["salience"], days, nature=m["nature"])
            ...
            fs = compute_hybrid_score(adj, tok_ov or 0.0, ww, rec_sc or 0.0, kw_scores.get(mid, 0), tag_Match or 0.0)
            if fs is None:
                fs = 0.0
        ...
        # P0.2: Enrich top results with graph context (facts + edges)
        top = await enrich_with_graph_context(top)
...
# ── P0.2: Fact Extraction Bridge ─────────────────────────────────────

async def _extract_facts_for_memory(memory_id: str, content: str, user_id: Optional[str] = None):
    """Fire-and-forget fact extraction for a newly created memory.
    
    Prefers cascaded entity extraction (P1.2) when available,
    falls back to regex-only SPO extraction (P0.2).
    """
    try:
        # Try cascaded extraction first (P1.2)
        from ..ops.extract_entities import extract_and_link_entities
        result = await extract_and_link_entities(memory_id, content, user_id)
        if result["fact_ids"]:
            logger = logging.getLogger("hsg")
            logger.debug(
                "Cascade: extracted %d entities, %d facts for memory %s",
                len(result["entities"]), len(result["fact_ids"]), memory_id[:16]
            )
            return
    except Exception as e:
        logger = logging.getLogger("hsg")
        logger.debug("Cascade extraction failed, falling back to regex: %s", e)

    # Fallback to regex-only extraction (P0.2)
    try:
        from ..ops.extract_facts import extract_and_link_facts
        fact_ids = await extract_and_link_facts(memory_id, content, user_id)
        if fact_ids:
            logger = logging.getLogger("hsg")
            logger.debug("Extracted %d facts for memory %s", len(fact_ids), memory_id[:16])
    except Exception as e:
        logger = logging.getLogger("hsg")
        logger.debug("Fact extraction skipped for memory %s: %s", memory_id[:16], e)


async def _create_temporal_edges(memory_id: str, mean_vec: List[float], ts: int, user_id: str = "anonymous", top_k: int = 5, min_sim: float = 0.5):
    """Create temporal edges between a new memory and its top-K nearest neighbors.

    Supplements waypoints (single best match) with broader temporal graph connectivity.
    """
    from ..temporal_graph import insert_edge

    mems = q.all_mem_by_user(user_id, 1000, 0) if user_id != "anonymous" else q.all_mem(1000, 0)
    nm = np.array(mean_vec, dtype=np.float32)

    scored = []
    for mem in mems:
        if mem["id"] == memory_id or not mem["mean_vec"]:
            continue
        ex = np.array(buf_to_vec(mem["mean_vec"]), dtype=np.float32)
        sim = float(cos_sim(nm, ex))
        if sim >= min_sim:
            scored.append((mem["id"], sim))

    scored.sort(key=lambda x: -x[1])
    for target_id, sim in scored[:top_k]:
        await insert_edge(memory_id, target_id, "semantic_neighbor", valid_from=ts, weight=sim, metadata={"source": "auto"})

    log = logging.getLogger("hsg")
    log.debug("Created %d temporal edges for memory %s", min(len(scored), top_k), memory_id[:16])


async def enrich_with_graph_context(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """P0.2: Enrich search results with knowledge graph context.
    
    For each top result, fetch its linked temporal facts and waypoint edges
    to provide graph-native context without changing the main search algorithm.
    """
    try:
        from ..temporal_graph import get_facts_for_memory
        ...
        for r in results:
            ...
            # Fetch linked facts via memory-fact bridge
            try:
                facts = await get_facts_for_memory(mem_id)
                r["graph_facts"] = facts[:5] if facts else []  # Top 5 facts per result
            except Exception:
                r["graph_facts"] = []
            
            # Fetch waypoint edges (already traversed in main query,
            # but expose them explicitly for graph-native consumers)
            try:
                edges = db.fetchall(
                    "SELECT dst_id, weight FROM waypoints WHERE src_id=? ORDER BY weight DESC LIMIT 5",
                    (mem_id,)
                )
                wp_edges = [
                    {"target": e["dst_id"], "weight": e["weight"], "type": "waypoint"}
                    for e in edges
                ]
            except Exception:
                wp_edges = []
            ...
            try:
                t_edges = db.fetchall(
                    "SELECT target_id, weight, relation_type FROM temporal_edges WHERE source_id=? AND valid_to IS NULL ORDER BY weight DESC LIMIT 5",
                    (mem_id,)
                )
                te = [
                    {"target": e["target_id"], "weight": e["weight"], "type": e["relation_type"]}
                    for e in t_edges
                ]
            except Exception:
                te = []
