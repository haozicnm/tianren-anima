...
async def apply_decay():
    ...
    for seg in segments:
        rows = db.fetchall("SELECT id,content,summary,salience,decay_lambda,last_seen_at,updated_at,primary_sector,nature,feedback_score as coactivations FROM memories WHERE segment=?", (seg,))
        ...
        for m in batch:
            ...
            mem_nature = dict_m.get("nature")
            if mem_nature and mem_nature in NATURE_DECAY_MODIFIERS:
                lam = lam * NATURE_DECAY_MODIFIERS[mem_nature]
