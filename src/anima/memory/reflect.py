...
async def run_reflection() -> Dict[str, Any]:
    ...
    # P2.1: After text-based reflection, also run graph reorganization
    if n == 0:
        # No text clusters found, try vector-based clustering
        try:
            from .graph_reorganizer import run_reorganization_once
            result = await run_reorganization_once()
            print(f"[REFLECT] Graph reorganizer: {result.get('status')}, {result.get('cluster_count', 0)} clusters")
        except Exception as e:
            print(f"[REFLECT] Graph reorganizer failed: {e}")
