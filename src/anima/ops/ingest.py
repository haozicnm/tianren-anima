...
async def mk_root(txt: str, ex: Dict, meta: Dict = None, user_id: str = None, nature: Optional[str] = None) -> str:
...
async def mk_child(txt: str, idx: int, tot: int, rid: str, meta: Dict = None, user_id: str = None, nature: Optional[str] = None) -> str:
    ...
    r = await add_hsg_memory(txt, json.dumps([]), m, user_id, nature)
    return r["id"]
...
async def ingest_document(t: str, data: Any, meta: Dict = None, cfg: Dict = None, user_id: str = None, tags: list = None, nature: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not use_rc:
        ...
        r = await add_hsg_memory(text, tags_json, m, user_id, nature)
        return {
            "root_memory_id": r["id"],
            "child_count": 0,
    ...
    cids = []
    try:
        rid_val = await mk_root(text, ex, meta, user_id, nature)
        for i, s in enumerate(secs):
             cid = await mk_child(s, i, len(secs), rid_val, meta, user_id, nature)
             cids.append(cid)
...
async def ingest_url(url: str, meta: Dict = None, cfg: Dict = None, user_id: str = None, nature: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not use_rc:
        ...
        m.update({"ingestion_strategy": "single", "ingested_at": int(time.time()*1000)})
        r = await add_hsg_memory(ex["text"], json.dumps([]), m, user_id, nature)
        return {
            "root_memory_id": r["id"],
            "child_count": 0,
