...
class SQLiteVectorStore(VectorStore):
    ...
    async def search(self, vector: List[float], sector: str, k: int, filter: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if filter and filter.get("user_id"):
            ...
            user_params = [filter["user_id"]]

        sql = f"SELECT id, v FROM {self.table} WHERE sector=? {filter_sql}"
        rows = db.conn.execute(sql, tuple(params)).fetchall()
        
        # Fallback: if vectors table is empty, use mean_vec from memories
        if not rows:
            fallback_sql = f"SELECT id, mean_vec as v FROM memories WHERE mean_vec IS NOT NULL {user_filter} LIMIT ?"
            from ..core.config import env
            rows = db.conn.execute(fallback_sql, (*user_params, k * 10)).fetchall()
        ...
        for r in rows:
            cnt = len(r["v"]) // 4
            stored_vec = np.array(struct.unpack(f"{cnt}f", r["v"]), dtype=np.float32)
            # Skip dimension mismatches silently
...
def get_vector_store() -> VectorStore:
    ...
    if backend == "postgres":
        dsn = os.getenv("OPENMEMORY_PG_DSN", "postgresql://user:***@localhost:5432/db")
        ...
        from .vector.zvec_store import ZvecVectorStore
        from .config import env
        dim = int(env.vec_dim) if hasattr(env, 'vec_dim') else 1024
        logger.info(f"Using ZvecVectorStore (HNSW index, dim={dim})")
        return ZvecVectorStore(dim=dim)
