...
class DB:
    ...
    def run_migrations(self):
        ...
        try:
            ...
            files = [p.name for p in resources.files('anima.migrations').iterdir() if p.name.endswith(".sql")]
...
class Queries:
    def ins_mem(self, **k):
        sql = """
        INSERT INTO memories(id, user_id, nature, segment, content, simhash, primary_sector, tags, meta, created_at, updated_at, last_seen_at, salience, decay_lambda, version, mean_dim, mean_vec, compressed_vec, feedback_score, parent_id, root_id)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ON CONFLICT(id) DO UPDATE SET
        user_id=excluded.user_id, nature=excluded.nature, segment=excluded.segment, content=excluded.content, simhash=excluded.simhash, primary_sector=excluded.primary_sector,
        ...
        """
        vals = (
            k.get("id"), k.get("user_id"), k.get("nature"), k.get("segment", 0), k.get("content"), k.get("simhash"),
